...
function run_floor()
    ...
    -- FBO stuff
    local vertex_src : String = read_file_as_string("data/shaders/screen.vp")
    local fragment_src : String = read_file_as_string("data/shaders/screen.fp")
    local screen_shader = create_shader( vertex_src, fragment_src )
    check_error("(screen) create shader", true)
    ...
    while loop_begin() do
		...
		local px:int = int(math.sin(2.0f*t)*64.0f)+128;
		local py:int = int(math.cos(3.0f*t)*64.0f)+128;
